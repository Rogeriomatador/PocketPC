...
class RuntimeDisplayBridgeProbeController(private val executionController:ProotExecutionController) {
    suspend fun execute(basePlan:ProotInvocationPlan,runtime:InstalledRuntime,tools:List<InstalledGuestTool>,layers:List<DeployedWindowsRuntimeLayer>,userApproved:Boolean,timeoutMillis:Long=15_000L):RuntimeDisplayBridgeProbeResult=coroutineScope {
        ...
        if(!userApproved||structural.isNotEmpty()||basePlan.argv.isEmpty()) {
            val p=executionController.executeOneShot(basePlan,userApproved,timeoutMillis)
        ...
        if(envErrors.isNotEmpty()) {
            val p=ProotExecutionResult(ProotExecutionState.BLOCKED,false,null,"",false,envErrors,"DISPLAY_BRIDGE_ENVIRONMENT_INVALID")
        ...
        val process=try{executionController.executeOneShot(plan,true,timeoutMillis)}finally{if(!accept.isCompleted)host.close()}
        ...
        if(peer!=null) {
            ...
            val interaction=runCatching {
                val create=peer.readFrame().getOrThrow(); machine.apply(create).getOrThrow()
                val geometry=peer.readFrame().getOrThrow(); machine.apply(geometry).getOrThrow()
                ...
                val destroy=peer.readFrame().getOrThrow(); machine.apply(destroy).getOrThrow()
