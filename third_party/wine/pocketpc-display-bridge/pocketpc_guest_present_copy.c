#include "pocketpc_guest_present_copy.h"

#include <stdlib.h>
#include <string.h>

static void pocketpc_guest_present_copy_reset(
...
static void pocketpc_guest_present_copy_destroy_idle(
    ...
    pocketpc_guest_present_copy_reset(submission);
}
...
int pocketpc_guest_present_copy_submit(
    ...
    if (
        ...
        destination->image == VK_NULL_HANDLE || !submission ||
        ...
        return POCKETPC_GUEST_PRESENT_COPY_INVALID_ARGUMENT;
    }

    if (source_format != destination_format)
        return POCKETPC_GUEST_PRESENT_COPY_FORMAT_MISMATCH;
    if (
        source_extent.width != destination_width ||
        source_extent.height != destination_height
    ) {
        return POCKETPC_GUEST_PRESENT_COPY_EXTENT_MISMATCH;
    }
    if ((destination_usage & VK_IMAGE_USAGE_TRANSFER_DST_BIT) == 0u)
        return POCKETPC_GUEST_PRESENT_COPY_DESTINATION_USAGE_MISSING;
    ...
    if (
        ...
        return POCKETPC_GUEST_PRESENT_COPY_FUNCTION_NOT_READY;
    }
    ...
    if (result != VK_SUCCESS) {
        free(wait_stages);
        pocketpc_guest_present_copy_reset(submission);
        return POCKETPC_GUEST_PRESENT_COPY_COMMAND_POOL_CREATE_FAILED;
    }
    ...
    if (result != VK_SUCCESS) {
        free(wait_stages);
        pocketpc_guest_present_copy_destroy_idle(device, submission);
        return POCKETPC_GUEST_PRESENT_COPY_COMMAND_BUFFER_ALLOCATE_FAILED;
    }
    ...
    if (result != VK_SUCCESS) {
        free(wait_stages);
        pocketpc_guest_present_copy_destroy_idle(device, submission);
        return POCKETPC_GUEST_PRESENT_COPY_SEMAPHORE_CREATE_FAILED;
    }
    ...
    if (result != VK_SUCCESS) {
        free(wait_stages);
        pocketpc_guest_present_copy_destroy_idle(device, submission);
        return POCKETPC_GUEST_PRESENT_COPY_COMMAND_BEGIN_FAILED;
    }
    ...
    if (result != VK_SUCCESS) {
        free(wait_stages);
        pocketpc_guest_present_copy_destroy_idle(device, submission);
        return POCKETPC_GUEST_PRESENT_COPY_COMMAND_END_FAILED;
    }
    ...
    if (result != VK_SUCCESS) {
        pocketpc_guest_present_copy_destroy_idle(device, submission);
        return POCKETPC_GUEST_PRESENT_COPY_QUEUE_SUBMIT_FAILED;
    }

    submission->active = 1;
    return POCKETPC_GUEST_PRESENT_COPY_OK;
}
...
int pocketpc_guest_present_copy_cleanup(
    ...
    if (result != VK_SUCCESS)
        return POCKETPC_GUEST_PRESENT_COPY_QUEUE_WAIT_FAILED;
